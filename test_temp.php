...
?>
  ...
  $testid=($_POST['take_test']);
  $tid=$_POST['take_test'];
  $uname=$_SESSION['username'];
  $sql="select count(*) as total from scorecard where testid='$testid' and user_name='$uname'";
  $srun=mysqli_query($connection,$sql);
  $attempts_finished=mysqli_fetch_array($srun);
  
  $r="select * from tests_data where test_id='$tid'";
  
  $run=mysqli_query($connection,$r);
  $rt=mysqli_fetch_array($run);
 
  if($rt['max_attempt']>$attempts_finished['total']){
  //$sql="insert into  scorecard (testid,user_name) values('$testid','$uname')";
  //$srun=mysqli_query($connection,$sql);
  $limit=(int)$rt['no_of_ques'];
  $time_limit=(int)$rt['time_limit'];
$q="SELECT * FROM test_questions where test_id='$tid' ORDER BY RAND() LIMIT ".$limit;
...
foreach( range('A', 'Z') as $elements) { 
...

 {
  ...
  ?>
	...
	//echo "<script>alert('max attempts reached.please contact admin'</script>";
	echo "<script>window.location.replace('index.php?error=max_attempts_limit_reached')</script>";
	}
	}
