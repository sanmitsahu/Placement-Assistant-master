    ...
    
    {
        ...
        if(mysqli_num_rows($qrun)>0){
            while($row1=mysqli_fetch_array($qrun))
            {
                ...
                if($dept=="Design skills")
                {
                   
                    $tmp01='<select class="form-control" id="skilltype" name="skilltype[]"><option selected>Design skills</option><option>Programming skills</option><option>Web skills</option><option>Others</option></select>';
                }
                else if($dept=="Programming skills")
                {
                    $tmp01='<select class="form-control" id="skilltype" name="skilltype[]"><option >Design skills</option><option selected>Programming skills</option><option>Web skills</option><option>Others</option></select>';
                }
                else if($dept=="Web skills")
                {
                    $tmp01='<select class="form-control" id="skilltype" name="skilltype[]"><option >Design skills</option><option>Programming skills</option><option selected>Web skills</option><option>Others</option></select>';
                }
                else if($dept=="Others")
                {
                    
                    $tmp01='<select class="form-control" id="skilltype" name="skilltype[]"><option >Design skills</option><option>Programming skills</option><option>Web skills</option><option selected>Others</option></select>';
                }
                $out1.=' <div>	<br><hr style="height:2px;border-width:100%;color:gray;background-color:gray">	<div class="input-group control-group after-add-more"> <span  class="col-md-5 col-sm-5 col-xs-5"><input type="text" name="skillinfo[]" id="skillinfo" class="form-control" placeholder="Enter your skills" value="'.$row1['skillinfo'].'" required/></span><label for="skilltype">Select Skill type:&nbsp</label>'.$tmp01.'</div><div id="error1" name="error1[]" style="color:#f23a3a;" ><?php// echo $error1; ?></div><br><div class="input-group control-group after-add-more " ><span  class="col-md-5 col-sm-5 col-xs-5"><input type="number" min="1" max="10" name="skilllvl[]" value="'.$row1['skilllvl'].'"  id="skilllvl" class="form-control" placeholder="Enter skill level" >&emsp;&emsp;*(Skill level range [1 - 10])</span><div id="error2" name="error2[]" style="color:#f23a3a;" ><?php// echo $error2; ?></div>&nbsp;&nbsp;&nbsp;&nbsp;<div><button class="btn ovrdbtn btn-outline-danger remove_field_1" type="button"><i class="fa fa-minus" style="font-size:20px"></i></button></div></div></div>';
        ...
        if(mysqli_num_rows($qrun)>0){
            ...
            while($row2=mysqli_fetch_array($qrun)){
                //echo "aaa";
                //echo $row2['coursect'];
                $out2.='	    <div><br><hr style="height:2px;border-width:100%;color:gray;background-color:gray"><div class="input-group control-group after-add-more">							<input type="text" readonly name="coursettl[]" id="coursettl" value="'.$row2['coursettl'].'" class="form-control" placeholder="Title">    </div>                        <div id="error3" name="error3[]" style="color:#f23a3a;padding-left:20px;margin:0px;padding-bottom:0px;padding-top:0px;" ><?php// echo $error2; ?></div>                       <br>					<div class="input-group control-group after-add-more">							<textarea name="coursedsc[]" id="coursedsc" class="form-control" placeholder="Description" value="" rows="4" cols="20">'.$row2['coursedsc'].'</textarea>					</div>  <br>				<div class="input-group control-group after-add-more">							Upload Certificate:  &nbsp;<a href="viewfile.php?location='.$row2['coursect'].'"><input type="file" id="coursect"  class="form-control" name="coursect[]" hidden>
                <input type="text" id="traverse"  class="form-control" value="t" name="traverse[]" hidden>
                <button class="btn  btn-outline-primary " type="button">view file</button></a>&nbsp;&nbsp; <button class="btn ovrdbtn btn-outline-danger remove_field_2" type="button"><i class="fa fa-minus" style="font-size:20px"></i></button>                           </div> <div id="error4" name="error4[]" style="color:#f23a3a;padding-left:20px;" ><?php// echo $error2; ?></div></div>&nbsp;&nbsp;'; //add input box
        }
        }	
            }
